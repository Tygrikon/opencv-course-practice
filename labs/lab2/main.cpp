...
int main(int argc, char* argv[])
{
  ...
  vector<Point2f> objKeypoint1(matches.size());
  vector<Point2f> sceneKeypoint2(matches.size());
  for (size_t matchInd = 0; matchInd < matches.size(); matchInd++)
  {
	  KeyPoint Object_keypoint=keypoints1[matches[matchInd].queryIdx];
	  KeyPoint Scene_keypoint = keypoints2[matches[matchInd].trainIdx];
	  float rad = (float)norm(Object_keypoint.pt - center);
	  float angle= atan( (Object_keypoint.pt.y - center.y) / (Object_keypoint.pt.x - center.x));

	  float delta_angel = (Scene_keypoint.angle - Object_keypoint.angle) * CV_PI / 180.0f;
	  float delta_scale = Scene_keypoint.size / Object_keypoint.size;

	  float scene_rad = delta_scale * rad;
	  float scene_angle = delta_angel + angle;

	      Point2f center_in_scene(
        Scene_keypoint.pt.x + scene_rad * cos(scene_angle),
        Scene_keypoint.pt.y + scene_rad * sin(scene_angle));
    hgt.at<int>( center_in_scene.y / splitSet , center_in_scene.x / splitSet ) += 1;
	objKeypoint1[matchInd] = keypoints1[matches[matchInd].queryIdx].pt;
	sceneKeypoint2[matchInd] = keypoints2[matches[matchInd].trainIdx].pt;
  ...
  Mat hommat = findHomography(objKeypoint1,sceneKeypoint2, CV_RANSAC);
  Mat warpImage;
